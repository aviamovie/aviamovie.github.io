(function () {
    ...
    function getSortTitle(sortId) {
        switch (sortId) {
            ...
            case 'first_air_date.desc': return Lampa.Lang.translate('sursSelect_first_air_date_desc');
            case 'vote_average.desc': return Lampa.Lang.translate('sursSelect_vote_average_desc');
            case 'popularity.desc': return Lampa.Lang.translate('sursSelect_popularity_desc');
            case 'revenue.desc': return Lampa.Lang.translate('sursSelect_revenue_desc');
            case 'vote_count.desc': return Lampa.Lang.translate('sursSelect_vote_count_desc');
            default: return '';
        }
    }
    ...
    function getLogoUrl(networkId, name, callback) {
        ...
        $.ajax({
            ...
            success: function (data) {
                callback(data?.logo_path ? Lampa.TMDB.image('t/p/w154' + data.logo_path) : 
    ...
    function updateLogo(networkId, name) {
        if (networkId) getLogoUrl(networkId, name, url => $(`.logo-${networkId}`).attr('src', url));
    }
    ...
    function showServiceList(serviceList) {
        var items = serviceList.map(service => ({
            title: `<div class="settings-folder" style="padding:0!important">${createLogoHtml(service.id, service.title)}</div>`,
            service: service
        }));
        ...
        serviceList.forEach(service => updateLogo(service.id, service.title));
    }

    // Выбор сортировки (ОСНОВНОЕ ИЗМЕНЕНИЕ)
    function showSortList(service) {
        var isMovie = service.url.startsWith('discover/movie');
        var sortItems = (isMovie ? sortOptionsMovie : sortOptionsTV).map(option => ({
            title: Lampa.Lang.translate(option.title),
            sort: applySortParams(option, {
        ...
        Lampa.Select.show({
            ...
            onSelect: function (sortItem) {
                ...
                Lampa.Activity.push({
                    ...
                    title: `${service.title} — ${Lampa.Lang.translate(sortItem.title)}`, // Формат "Название — Сортировка"
            ...
            onBack: function () {
                service.url.includes('with_networks=') ? showStreamingTypeMenu() : showSursSelectMenu();
            }
        });
    }
    ...
    function initPlugin() {
        var icon = `<svg fill="currentColor" height="200px" width="200px" version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
        ...
        $('.menu .menu__list').eq(0).append(
            $(`<li class="menu__item selector" data-action="streaming">
                <div class="menu__ico">${icon}</div>
                <div class="menu__text">${Lampa.Lang.translate('sursSelect_menu_item')}</div>
            </li>`).on('hover:enter', showSursSelectMenu)
        );
    }
