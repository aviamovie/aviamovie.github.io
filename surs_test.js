...
function addMainButton() {
    ...
    $('.menu .menu__list').eq(0).append(button);
        ...
        var buttonsData = [
            ...
            { id: 'btn2', title: 'Кнопка 2', icon: './img/icons/bookmark.svg' },
            { id: 'btn3', title: 'Кнопка 3', icon: './img/icons/bookmark.svg' },
            { id: 'btn4', title: 'Кнопка 4', icon: './img/icons/bookmark.svg' }
        ];
        ...
        function renderMainButtons() {
            ...
            // Используем Lampa.Arrays.insert для добавления кнопок в массив
            var buttonItems = [];
            Lampa.Arrays.insert(buttonItems, 0, buttonsData);

            buttonItems.forEach(function (button) {
                var $button = Lampa.Template.js('register').addClass('selector').addClass('main-button-' + button.id);
                $button.find('.register__name').text(button.title);
                $button.find('.register__counter').html('<img src="' + button.icon + '" style="height: 2.2em; padding: 0.4em;" />');

                $button.on('hover:enter', function () {
                    console.log('Нажата кнопка: ' + button.title);
                    // Добавить функционал позже
                });

                $buttonRow.append($button);
            });

            $render.prepend($buttonRow);
            Lampa.Controller.collectionSet($buttonRow);
            Lampa.Controller.collectionFocus($buttonRow.find('.selector').first(), $render);
        }
        ...
        $('<style>').prop('type', 'text/css').html(`
            ...
            .main-buttons-row .register__counter {
                display: flex;
